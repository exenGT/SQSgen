...
def findNNdist(cell_params, cell_angles_rad, frac_coords):
    ...
    ### group all x, y, z coordinates into bins
    frac_min = np.array([1., 1., 1.])
    frac_max = np.array([0., 0., 0.])

    for r_frac in frac_coords:
        for i in range(3):

            if r_frac[i] < frac_min[i]:  frac_min[i] = r_frac[i]
            if r_frac[i] > frac_max[i]:  frac_max[i] = r_frac[i]

    L_frac = frac_max - frac_min

    L_abc = L_frac * cell_params

    print(L_abc)

    # L_frac_max = np.amax(L_frac)

    # L_ratio = L_frac / L_frac_max
    # L_ratio_tol = 1e-2

    # for i in range(3):
    #     if L_ratio[i] < L_ratio_tol:
    #         L_ratio[i] = 0

    # L_ratio_nonzero = L_ratio[np.nonzero(L_ratio)]
    vol = np.prod(L_abc)

    ### determine the subcell dimensions and number of subcells inside the supercell
    N = len(frac_coords)

    # Nc_max = np.power( N / L_ratio_prod, 1. / len(L_ratio_nonzero) )

    L_sub = np.power(vol/N, 1./3)

    ### determine the subcell dimensions and number of subcells inside the supercell
    Lc = [L_sub / np.amin(np.sin(np.delete(cell_angles_rad, i))) for i in range(3)]
    Nc = np.floor(L_abc / Lc)
    ...
    ### assign each atom to its respective subcell
    cells = [[] for cell in range(int(np.prod(Nc)))]
    cells_at_num = np.zeros(int(np.prod(Nc))).astype(int)

    for at_ind in range(N):    # atom index
        r = frac_coords[at_ind]
        c_r = int(Nc[2] * Nc[1] * np.floor((r[0] - frac_min[0])*Nc[0]) + Nc[2] * np.floor((r[1] - frac_min[1])*Nc[1]) + np.floor((r[2] - frac_min[2])*Nc[2]))
        cells[c_r].append(at_ind)
        cells_at_num[c_r] += 1

    cells = np.asarray(cells)

    cells_nonEmpty = cells[np.nonzero(cells_at_num)]
    cells_nonEmpty_ind = []

    for cell_ind in np.nonzero(cells_at_num)[0]:    # for all non-empty cells

            I = np.floor(cell_ind / (Nc[2] * Nc[1]))
            J = np.floor((cell_ind - Nc[2] * Nc[1] * I) / Nc[2])
            K = cell_ind - Nc[2] * Nc[1] * I - Nc[2] * J

            cells_nonEmpty_ind.append([I, J, K])
    
    cells_nonEmpty_compos_ind = np.nonzero(cells_at_num)[0]

    print(cells)
    #print(cells_nonEmpty_compos_ind)
    #print(cells_nonEmpty_ind)

    # c_maxAt = cells_at_num.argmax()
    # maxAtnum = cells_at_num[c_maxAt]

    # I = np.floor(c_maxAt / (Nc[2] * Nc[1]))
    # J = np.floor((c_maxAt - Nc[2] * Nc[1] * I) / Nc[2])
    # K = c_maxAt - Nc[2] * Nc[1] * I - Nc[2] * J
    ...
    for c_id_1 in range(len(cells_nonEmpty)):
        for c_id_2 in range(c_id_1, len(cells_nonEmpty)):

            if (np.abs(cells_nonEmpty_ind[c_id_2][0] - cells_nonEmpty_ind[c_id_1][0]) <= 1) \
                and (np.abs(cells_nonEmpty_ind[c_id_2][1] - cells_nonEmpty_ind[c_id_1][1]) <= 1) \
                and (np.abs(cells_nonEmpty_ind[c_id_2][2] - cells_nonEmpty_ind[c_id_1][2]) <= 1):

                    for at_1 in cells_nonEmpty[c_id_1]:
                        for at_2 in cells_nonEmpty[c_id_2]:
            
                            if at_2 != at_1:
            
                                Dist.append(np.linalg.norm(Cts_coords[at_1] - Cts_coords[at_2]))
                                cell_pair.append(cells_nonEmpty_ind[c_id_1] + cells_nonEmpty_ind[c_id_2])
                                at_pair.append((at_1, at_2))
    ...
    #             if (c_ijk >= 0) and (c_ijk < np.prod(Nc)):

    #                 for at_1 in cells[c_maxAt]:
    #                     for at_2 in cells[c_ijk]:

    #                         if (at_2 > at_1) or (c_ijk != c_maxAt):

    #                             Dist.append(np.linalg.norm(Cts_coords[at_1] - Cts_coords[at_2]))
    #                             cell_pair.append((I, J, K, i, j, k))
    #                             at_pair.append((at_1, at_2))
    ...
    minDist_ind = Dist.argmin()

    print(cell_pair[minDist_ind])

    print(at_pair[minDist_ind])
